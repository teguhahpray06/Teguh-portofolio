...
const Experience = ({ index, company, position, desc, institute, degree, startDate, endDate }: ExperienceProps) => {
  ...
  return (
    <div className={`mb-6 md:mb-8 flex md:justify-between items-center w-full ${index % 2 === 0 ? 'md:flex-row-reverse left-timeline' : 'right-timeline'}`}>
      ...
      <span className="z-20 flex items-center order-1 justify-center w-6 h-6 md:w-9 md:h-9 ml-3 md:ml-0 bg-green-200 rounded-full ring-4 md:ring-8 ring-white dark:ring-grey-800 dark:bg-green-600">
      ...
      <motion.div
        ...
        className="interactive-lift order-1 rounded-lg w-full ml-3 md:ml-0 bg-white dark:bg-grey-800 md:w-5/12 p-3 md:px-4 md:py-4 border border-transparent hover:border-green-500/30">
